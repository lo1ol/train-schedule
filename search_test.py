...
def search_test(log='log_search.txt', trace=False):
    ...
    for i in (int((10 ** (i // 2)) / 2) if i % 2 == 0 else (10 ** (i // 2)) for i in range(2, 15)):
        ...
        print('Linear search on %-7s records = %-22s sec' % (i, linear_sum / 10), file=logfile)
        print('Binary search on %-7s records = %-22s sec' % (i, binary_sum / 10), file=logfile)
        print('Map search on %-7s records = %-22s sec' % (i, map_sum / 10), file=logfile)
        if trace:
            print('Linear search on %-7s records = %-22s sec' % (i, linear_sum / 10))
            print('Binary search on %-7s records = %-22s sec' % (i, binary_sum / 10))
            print('Map search on %-7s records = %-22s sec' % (i, map_sum / 10))
...
if __name__ == "__main__":
    ...
    parser.add_option('-l', '--log', type='string', help='Logging measure in to FIlE, default in log.txt',
